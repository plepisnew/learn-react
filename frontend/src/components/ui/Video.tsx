...
const Video: React.FC<Props> = (props: Props) => {
    ...
    if (typeof props.url === 'string')
        return props.url.startsWith('/')
            ? createLink(props.url, createVideo())
            : createAnchor(props.url, createVideo());

    const randomVideo = props.url[Math.floor(Math.random() * props.url.length)];
    return randomVideo.startsWith('/')
        ? createLink(randomVideo, createVideo())
        : createAnchor(randomVideo, createVideo());
};
