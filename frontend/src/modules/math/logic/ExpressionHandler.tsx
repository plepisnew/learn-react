...
const compare = (op1: string, op2: string): number => {
    const val1 = math.precedence.indexOf(op1);
    const val2 = math.precedence.indexOf(op2);
 ...
 */
const stringToExpression = (
    expr: string | (string | Expression)[]
): Expression => {
    const exprArray: (string | Expression)[] =
        typeof expr === 'string' ? expr.split('') : expr;
    // deepest level never updated?
    const deepestLevel = 0;
    let deepestIndex = 0;
    let level = 0;
        ...
        if (operatorIndex != -1) {
            ...
            const exp1 =
                typeof firstElement === 'string'
                    ? new Symbol(
                          firstElement,
                          parseInt(firstElement) || undefined
                      )
                    : firstElement;
            ...
            const exp2 =
                typeof secondElement === 'string'
                    ? new Symbol(
                          secondElement,
                          parseInt(secondElement) || undefined
                      )
                    : secondElement;
...
const expressionToClientText = (expr: Expression): React.ReactNode => {
    const createContent = () => {
        ...
        const exponent = (a: string, b: string) => (
            <>
                {a}
                <sup>{b}</sup>
            </>
        ...
        return (
            <span>
                {expr.operator.symbol === '^'
                    ? exponent(
                          expr.operands[0].getRepresentation(),
                          expr.operands[1].getRepresentation()
                      )
                    : normal(expr.getRepresentation())}
            </span>
        );
    };
