...
const DevPage: React.FC = () => {
    ...
    const exp: Expression = Expression.builder().operand(x).operator('^').operand(x).build();
...
const dev = () => {
    ...
    let x_squared = Expression.builder()
                            .operand(x)
                            .operand(two)
                            .operator(Operator.get('exp'))
                            .build();
    
    let y_squared = Expression.builder()
                            .operand(y)
                            .operand(two)
                            .operator(Operator.get('exp'))
                            .build();

    let diff = Expression.builder()
                            .operand(x_squared)
                            .operator(Operator.get('minus'))
                            .operand(y_squared)
                            .build();

    let half = Expression.builder()
                            .operand(one)
                            .operator(Operator.get('/'))
                            .operand(two)
                            .build();

    let dist = Expression.builder()
                            .operand(diff)
                            .operand(half)
                            .operator(Operator.get('exp'))
                            .build();

    let seventh = Expression.builder()
                            .operand(one)
                            .operand(seven)
                            .operator(Operator.get('/'))
                            .build();
