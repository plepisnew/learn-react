...
class Expression {
    ...
    calculate = (): Expression => {
        ...
        Object.keys(this.unknowns).forEach(key => isCalculable = isCalculable && !(this.unknowns[key]));
        
        if(mathDebug.logCalculation) console.log(`Performing Calculation on ${isCalculable ? 'known' : 'unknown'} Expression`, this);
        ...
        if(isCalculable) {
            ...
            let integerDecimalArray = `${this.value}`.split('.');
            let isDecimalNumber = integerDecimalArray.length != 1;
            let decimalDigits = (integerDecimalArray[1] || '0').length;
            // if not decimal or if not over threshold, then replace. otherwise keep the same
            if(!(isDecimalNumber && decimalDigits > mathDebug.lazyCalculationDecimalDigits)) {
    ...
    getUnknowns = (): { [key: string]: boolean } => {
        ...
        this.operands.forEach(operand => {
            if(operand.constructor.name === 'Symbol' && operand.value == undefined) {
...
class ExpressionBuilder {
    ...
    operator = (bOperator: Operator | string): ExpressionBuilder => {
        this.bOperator = typeof bOperator === 'string' ?  Operator.get(bOperator) : bOperator;
        return this;
    }
