...
class OperatorBuilder {
    ...
    calculationFunction = (bCalculationFunction: (values: number[]) => number) => {
        this.bCalculationFunction = bCalculationFunction;
        return this;
    }

    representationFunction = (bRepresentationFunction: (values: string[]) => string) => {
...
class OperatorFactory {
    ...
    static addition = (): Operator => {
        return Operator.builder()
                    ...
                    .representationFunction((values: string[]) => `(${values[0]}+${values[1]})`)
                    .build();
    }
    ...
    static exponentiation = (): Operator => {
        return Operator.builder()
                    .symbol('^')
                    .name('exponentiation')
                    .calculationFunction((values: number[]) => Math.pow(values[0], values[1]))
                    .representationFunction((values: string[]) => `(${values[0]}^${values[1]})`)
                    .build();
    }
...
const getOperator = (operator: string): Operator => {
    ...
    if(['+', 'plus', 'add', 'addition', 'sum', 'summation'].includes(operator)) return operators.add;
    if(['-', 'minus', 'sub', 'subtract', 'subtraction'].includes(operator)) return operators.sub;
    if(['*', 'multiply', 'multi', 'multiplication'].includes(operator)) return operators.multi;
    if(['/', ':', 'division', 'div', 'divide'].includes(operator)) return operators.div;
    if(['^', 'exp', 'e', 'exponent', 'exponentiation'].includes(operator)) return operators.exp;
